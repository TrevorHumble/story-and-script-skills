...
def main():
    ...
    for label, frames in sequences:
        first_dir = os.path.dirname(frames[0])
        files = [{"name": os.path.basename(f)} for f in frames]
        ...
        # Add the rest of the frames to the strip
        for f in frames[1:]:
    ...
    # Output placeholder — the user can change this when rendering the final cut
